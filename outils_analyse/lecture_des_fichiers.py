...
def lire_csv_a_3_colonnes(path_complet_vers_csv: str or PathLike,
    ...
    :param delimier: caractère séparant les valeurs d'une même rangé
...
def crop_pour_conserver_que_la_partie_avec_rampe(valeurs: np.array, indice_colonne_rampe: int, zero_threshold: float,
                                                 infinity_threshold: float) -> np.array:
    """
    Cette méthode permet de retirer tous les éléments de l'array où le gradient de valeur de tension de la rampe
    associé est nul* ou inifni**.
